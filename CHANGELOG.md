...
- Wire plate AI through OpenRouter (`OPENROUTER_API_KEY` / `AI-KEY`), load monorepo-root `.env` from the server cwd, keep stub fallback.
- Retarget camera scan: plate photo → description/nutrition/gaps saved per user, plus Weekly review tab from meal logs (stub without an AI key).
...
- Polish PlateWise UI: liquid glass panels, Anthropic-inspired plate logo, softer layered animated arc atmosphere, wellness botanical backdrop (EN/NL).
